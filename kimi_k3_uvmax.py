...
#
# Based on Tarjei Mandt's Kimi-K3 implementation for mlx-lm PR #1626
# (commit 7d505c285b801108a52c23353c7fb6af07204717), retained under
# mlx-lm's MIT license.  This fork adds stable-release compatibility and a
# correct odd-layer pipeline partition for local, two-node UVMAX checkpoints.
...
def _compute_g_safe(A_log, a, dt_bias, lower_bound):
    """K3's bounded decay from mlx-lm PR #1626.

    mlx-lm 0.31.3 has the optimized gated-delta kernel but its public helper
    does not yet accept ``lower_bound``.  Keeping the tiny compatibility shim
    here lets the project stay on the latest stable release without falling
    back to the much slower reference recurrence.
    """
    return mx.exp(
...
def _k3_gated_delta_update(
    ...
    if lower_bound is None:
        ...
        g = _compute_g_safe(A_log, a, dt_bias, lower_bound)
    if state is None:
...
def pipeline_bounds(num_layers: int, size: int, rank: int) -> tuple[int, int]:
    """Return a gap-free reverse-pipeline layer interval for ``rank``."""
...
class KimiK3DeltaAttention(nn.Module):
    ...
    def _decode_core(self, x, conv_state, ssm_state):
        ...
        out, ssm_state = _k3_gated_delta_update(
            q,
            k,
            v,
            a_logits,
...
class KimiK3TextModel(PipelineMixin, nn.Module):
    def __init__(self, args: TextArgs):
        super().__init__()
        self.args = args
    ...
    def pipeline(self, group):
        ...
        ranks this assigns layers [0, 47) to rank 1 and [47, 93) to rank 0,
        balancing the 92 expensive MoE layers exactly.
        """
    ...
    def __call__(
        ...
        if rank == 0 and blocks is not None:
